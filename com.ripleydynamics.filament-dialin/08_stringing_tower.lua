...
function execute(opts)
    ...
    local temps
    if opts.by_interval and interval == 0 then
    ...
    util.log(string.format("stringing tower for %s: %d sections, %d C down to %d C, fan %s, travel %s mm", tag, n, temps[1], temps[n],
    ...
    util.data(bed, "string", { tag = tag, values = util.join(temps, 0), sections = n, fan_start = fan_start, fan_step = fan_step })
end
