...
-- One text field instead of the five (min, max, by_interval, interval, count)
-- the dialog used to need: alpha11 has no dropdowns and no conditional fields,
-- so a sweep is typed the way it is spoken.
--
-- Accepted, case-insensitively and with flexible whitespace:
--   "A to B step S"   "A-B step S"   "A..B step S"   by interval
--   "A to B x N"      "A to B in N"  "A to B / N"    by number of values
--   "A"                                              a single value
-- Values may be decimals, negative, or percentages ("10% to 35% step 5%",
-- which strips the signs and reports percent = true).
local NUMBER = "[-+]?%d*%.?%d+%%?"
local NUM_CAPTURE = "(" .. NUMBER .. ")"
local RANGE_FORMS = " Accepted forms: 'A to B step S' (also 'A-B step S' or 'A..B step S'), "
    .. "'A to B x N' (also 'in N' or '/ N'), or a single number 'A'."

-- { pattern, by_interval } in the order they are tried.
local RANGE_TAILS = {
    { "^(.-)%s*step%s*" .. NUM_CAPTURE .. "$", true },
    { "^(.-)%s*x%s*" .. NUM_CAPTURE .. "$", false },
    { "^(.-)%s*in%s*" .. NUM_CAPTURE .. "$", false },
    { "^(.-)%s*/%s*" .. NUM_CAPTURE .. "$", false },
}
local RANGE_SEPARATORS = {
    "^" .. NUM_CAPTURE .. "%s*to%s*" .. NUM_CAPTURE .. "$",
    "^" .. NUM_CAPTURE .. "%s*%.%.%s*" .. NUM_CAPTURE .. "$",
    "^" .. NUM_CAPTURE .. "%s*%-%s*" .. NUM_CAPTURE .. "$",
}

-- Parses a typed range and expands it with M.range, so rounding, the integer
-- flag, max_count and the range validations are exactly the ones the five
-- fields used to get. Options: o.integer, o.max_count, o.what (name used in
-- the assertion messages).
-- Returns the value list in the order typed (descending when A > B) and the
-- spec { lo, hi, by_interval, interval, count, percent }.
function M.parse_range(text, o)
    o = o or {}
    local what = o.what or "Range"
    local s = text == nil and "" or tostring(text):lower():gsub(",", ".")
    s = s:gsub("^%s+", ""):gsub("%s+$", ""):gsub("%s+", " ")
    assert(s ~= "", what .. " must not be empty." .. RANGE_FORMS)

    local percent = false
    local function value(token, name)
        local body = token:match("^(.-)%%$")
        if body then
            percent = true
            token = body
        end
        return M.num(token, name)
    end
    ...
    local body, tail_token, by_interval
    for _, t in ipairs(RANGE_TAILS) do
        local b, n = s:match(t[1])
        if b then
            body, tail_token, by_interval = b, n, t[2]
            break
        end
    end

    if body == nil then
        -- A single value is the only form without a step or a count.
        local only = s:match("^" .. NUM_CAPTURE .. "$")
        assert(only, what .. " could not be read: '" .. s .. "'. A range needs a step or a count." .. RANGE_FORMS)
        local v = round(value(only, what))
        return { v }, { lo = v, hi = v, by_interval = false, interval = 0, count = 1, percent = percent }
    end

    local lo_token, hi_token
    for _, p in ipairs(RANGE_SEPARATORS) do
        local a, b = body:match(p)
        if a then
            lo_token, hi_token = a, b
            break
        end
    end
    assert(lo_token, what .. " could not be read: '" .. s .. "'." .. RANGE_FORMS)

    local lo = value(lo_token, what .. " start")
    local hi = value(hi_token, what .. " end")
    local step_or_count = value(tail_token, by_interval and (what .. " step") or (what .. " count"))
    local values, interval = M.range {
        min = math.min(lo, hi), max = math.max(lo, hi),
        by_interval = by_interval, interval = step_or_count, count = step_or_count,
        integer = o.integer, max_count = o.max_count,
    }
    if lo > hi then -- keep the direction the user typed; M.range ascends
        for i = 1, math.floor(#values / 2) do
            values[i], values[#values + 1 - i] = values[#values + 1 - i], values[i]
        end
    end
    return values, { lo = lo, hi = hi, by_interval = by_interval, interval = interval, count = #values, percent = percent }
end
